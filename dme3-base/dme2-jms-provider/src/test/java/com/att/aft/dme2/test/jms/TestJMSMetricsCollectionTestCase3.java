...
@Ignore
public class TestJMSMetricsCollectionTestCase3 extends JMSBaseTestCase{
    ...
    @Test
    public void testMetricsJMSClientWithPartner() throws Exception {
		System.setProperty("AFT_DME2_PUBLISH_METRICS", "false");
		System.setProperty("AFT_DME2_DISABLE_METRICS", "false");		
		
		System.setProperty("DME2.DEBUG", "true");
		String containerName="com.att.test.MetricsTestCollectorClientWithPartner"; 
		...
		DefaultMetricsCollector collector = DefaultMetricsCollector.getMetricsCollector(containerName, containerVersion, containerRO, containerEnv, 
				containerPlat, containerHost, containerPid, "METRICS_PARTNER");
...
/**		Timeslot slot = collector.getTimeslotForTime(eventTime);
		...
		while(it.hasNext()) {
			...
			assertTrue(svc.getContainer().getPartner().equals("METRICS_PARTNER"));
			return;
		}
*/
