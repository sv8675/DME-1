...
import com.att.aft.dme2.test.jms.util.ContinuationQueueTestMessageListener;
...
public class TestContinuationReplyServer {
	...
	public TestContinuationReplyServer() throws Exception {
	}
	...
	public void init() throws Exception {
		...
		requestQueue = (Queue) context.lookup(TestConstants.continuationReplyServiceToRegister);
		ContinuationQueueTestMessageListener[] listeners = new ContinuationQueueTestMessageListener[TestConstants.listenerCount];
		for (int i = 0; i < listeners.length; i++) {
			listeners[i] = new ContinuationQueueTestMessageListener(context, connection, session, requestQueue);
			listeners[i].start();
		}
		System.out.println("TestContinuationReplyServer.init(): requestQueue.getListeners().size()="
				+ ((DME2JMSLocalQueue) requestQueue).getListeners().size());

		System.out.println("TestContinuationReplyServer started successfully...");
		...
		f.delete();
		System.out.println("TestContinuationReplyServer destroyed.");
	}

	public static void main(String[] args) throws Exception {
		TestContinuationReplyServer server = new TestContinuationReplyServer();
