...
public class TestEmptyReplyJMSServer {
	...
	public TestEmptyReplyJMSServer() throws Exception {
	}
	...
	public void init() throws Exception {
		...
		requestQueue = (Queue) context.lookup(TestConstants.emptyReplyServiceToRegister);
		TestEmptyReplyListener[] listeners = new TestEmptyReplyListener[TestConstants.listenerCount];
		for (int i = 0; i < listeners.length; i++) {
			listeners[i] = new TestEmptyReplyListener(connection, session, requestQueue);
			listeners[i].start();
		}
		System.out.println("TestEmptyReplyJMSServer.init(): requestQueue.getListeners().size()="
				+ ((DME2JMSLocalQueue) requestQueue).getListeners().size());

		System.out.println("TestEmptyReplyJMSServer started successfully...");
		...
		f.delete();
		System.out.println("TestEmptyReplyJMSServer destroyed.");
	}

	public static void main(String[] args) throws Exception {
		TestEmptyReplyJMSServer server = new TestEmptyReplyJMSServer();
