...
public class TestGRMRegJMSServer {
	...
	public TestGRMRegJMSServer() throws Exception {
	}
    ...
    if (city.equals("BHAM"))
		...
		requestQueue = (Queue) context.lookup(serviceName != null ? serviceName : TestConstants.serviceToRegister);
	...
	public static void main(String[] args) throws Exception {
		TestGRMRegJMSServer server = new TestGRMRegJMSServer();
		...
		if (args.length == 0)
			...
			for (int i = 0; i < args.length; i++) {
				...
				else if ("-service".equals(args[i]))
					server.setServiceName(args[i + 1]);
			}
		}
