...
import com.att.aft.dme2.test.jms.util.TestMetricsMessageListener;

public class TestMetricsJMSServer {
	...
	public TestMetricsJMSServer() throws Exception {
	}
	...
	public void init() throws Exception {
		...
		TestMetricsMessageListener[] listeners = new TestMetricsMessageListener[TestConstants.listenerCount];
		for (int i = 0; i < listeners.length; i++) {
			listeners[i] = new TestMetricsMessageListener(connection, session, requestQueue);
			listeners[i].start();
		}
		System.out.println("TestMetricsJMSServer.init(): requestQueue.getListeners().size()="
				+ ((DME2JMSLocalQueue) requestQueue).getListeners().size());

		System.out.println("TestMetricsJMSServer started successfully...");
		...
		f.delete();
		System.out.println("TestMetricsJMSServer destroyed.");
	}

	public static void main(String[] args) throws Exception {
		TestMetricsJMSServer server = new TestMetricsJMSServer();
