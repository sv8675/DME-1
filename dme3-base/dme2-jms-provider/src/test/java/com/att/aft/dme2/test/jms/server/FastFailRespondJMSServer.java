...
public class FastFailRespondJMSServer {
	...
	public FastFailRespondJMSServer() throws Exception {
	}
	...
	public void start() throws Exception {
		...
		FastFailoverMessageListener[] listeners = new FastFailoverMessageListener[20];
		for (int i = 0; i < listeners.length; i++) {
			listeners[i] = new FastFailoverMessageListener(connection, requestQueue, sPort);
			listeners[i].start();
		}
		System.out.println("FastFailRespondJMSServer. request queue is: "+requestQueue.getQueueName()+" connection is: "+connection.toString()+" port is: "+sPort+" listener count is: "+listeners.length);		
		System.out.println("FastFailRespondJMSServer.init(): requestQueue.getListeners().size()="
				+ ((DME2JMSLocalQueue) requestQueue).getListeners().size());

		System.out.println("FastFailRespondJMSServer started successfully...");
		...
		for (int i = 0; i < listeners.length; i++) {
			try {
				...
				// Ignore any error in stopping listener
			}
		}

		session.close();
		connection.close();
		System.out.println("FastFailRespondJMSServer destroyed.");
		System.exit(0);
	}

	public static void main(String[] args) throws Exception {
		try {
			FastFailRespondJMSServer server = new FastFailRespondJMSServer();
