...
/**
 *
 * parsed information about resource DME connecting to
 * you can find some information here about URI pattern passed to constructor of this class:
 *
 * http://wiki.web.att.com/display/soacloud/DME2+-+URI+Patterns
 *
 */
public class DME2UniformResource {
  ...
  private Float throttlePctPerPartner = null;
	...
	// this field can be used to override version in the URL
  private String preferredVersion;
	...
	public DME2UniformResource(DME2Configuration config, String protocol, String host, int port, String path, URLStreamHandler handler)
	...
	public DME2UniformResource(DME2Configuration config, URL inURL) {
		url = inURL;
		host = inURL.getHost();
		urlType = getDME2URLType(inURL.getHost());
		path = inURL.getPath();
		this.config = config;
	...
	private DME2UrlType getDME2URLType(String host) {
		 ...
		 */
			...
			type = DME2UrlType.DIRECT;
		}

		return type;
	}

  // @TODO: optimize this getField, parse the parameters once instead of for each getField
	private void processResourcePath(String inPath) {
		...
		if ((service == null) || (version == null && envContext == null)) {
			...
			// version/envContext/routeOffer/partner info are available from
			// there.
			...
			// If serviceName is not derived in above steps, then the URI is of
			// type STANDARD
			// where hostname will be used as service.
			if (this.urlType != DME2UrlType.RESOLVABLE && this.urlType != DME2UrlType.SEARCHABLE) {
				if (version != null && envContext != null && (routeOffer != null || partner != null)
						&& service == null) {
					...
					if (this.bindContext != null) {
						cpath = this.bindContext.startsWith("/")
								? this.bindContext.substring(1, this.bindContext.length()) : this.bindContext;
					}
					...
					// If the service name is successfully retrieved from the
					// URI hostname, then its using standard web address/uri
					if (this.service != null && port <= 0) {
						urlType = DME2UrlType.STANDARD;
					}
				}
			}
		}
				...
				if (pair.length == 2) {
					...
					if (key.equalsIgnoreCase("endpointreadtimeout")) {
						try {
							...
							// maximum
							// defined ( 5 mins )
							this.endpointReadTimeout = Math.min(Long.parseLong(value),
									config.getLong(DME2Constants.AFT_DME2_EP_READ_TIMEOUT));
						} catch (Exception e) {
							logger.debug(null, "processQueryString", LogMessage.DEBUG_MESSAGE, "Exception", e);
							/* ignore error in parsing */ }
					} else if (key.equalsIgnoreCase("roundtriptimeout")) {
						try {
							// validate if roundTripTimeout is more than maximum
							// defined ( 4 mins )
							this.roundTripTimeout = Math.min(Long.parseLong(value),
									config.getLong(DME2Constants.AFT_DME2_DEF_ROUNDTRIP_TIMEOUT_MS));
						} catch (Exception e) {
						...
						try {
							...
							// defined ( 4 secs )
							this.connectTimeout = Math.min(Long.parseLong(value),
									config.getLong(DME2Constants.AFT_DME2_EP_CONN_TIMEOUT));
						} catch (Exception e) {
							logger.debug(null, "processQueryString", LogMessage.DEBUG_MESSAGE, "Exception", e);
							/* ignore error in parsing */ }
						...
						try {
							// validate if connectTime is more than maximum
							// defined ( 4 secs )
							this.wsConnIdleTimeout = Integer.parseInt(value) == 0
									? config.getInt(DME2Constants.AFT_DME2_DEF_WS_IDLE_TIMEOUT)
						...
						try{
							this.throttlePctPerPartner = Float.parseFloat(value);
						} catch(Exception exception){
							//this.throttlePctPerPartner = config.getFloat( DME2Constants.AFT_DME2_THROTTLE_PCT_PER_PARTNER );
							logger.debug( null, "processQueryString",  LogMessage.DEBUG_MESSAGE, "Error parsing Throttle percent per partner.",exception);
						}
					}
						...
						try{
							...
							logger.debug( null, "processQueryString", LogMessage.DEBUG_MESSAGE, "Error parsing Throttle filter disabled query parameter",exception);
						}
					}
					else {
						if (queryParams == null) {
	...
	public void assertValid() throws DME2Exception {
		...
		if (version == null) {
      // @TODO: Check what if preferred_verison is not null
	...
	private String getField(String servletPath, String field, boolean canEndWithSlash) {
		...
		if (indexOfField != -1) {
			try {
				...
				// Identify the next field using = sign. Requirement is all
				// input keys will only have = appended to it.
				...
				if (indexOfNext != -1) {
					...
					// Most of the fields cannot end with / in it except
					// serviceName or subContext fields
					// Identify if servletPath ends with / for a field that
					// should not carry / in it still
					// E.g
					// service=com.att.csi.m2e.Echo/version=1.0.0/envContext=TEST/
				...
				// If values carry "/" in it for fields that do not have /
				// supported in its name E.g version,
				// envContext,partner,routeOffer
				// replace all / with ""
				if (!canEndWithSlash) {
				...
				// If field is not null, contains / , only once and at the end
				// of the string, then replace that with blank
				// E.g
				// DME2SEARCH/version=1.0/envContext=PROD/service=com.att.aft.MyService/
				if (fieldVal != null && fieldVal.contains("/")
						&& (fieldVal.replaceAll("/", "").length() == fieldVal.length() - 1)
						&& fieldVal.indexOf("/") == fieldVal.length() - 1) {
					fieldVal = fieldVal.replace("/", "");
				}
				return fieldVal != null ? (fieldVal.length() > 0 ? fieldVal : null) : null;
			} catch (Exception e) {
				// Return null for any unhandled exception in parsing the fields
